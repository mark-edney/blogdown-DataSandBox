...
accelerometers. More information on the project can be found [here](https://web.archive.org/web/20161224072740/https:/groupware.les.inf.puc-rio.br/har).
...
Some trimming is required. The first trim is performed with the near zero variance function from the caret library, which finds the predictors that exhibit near zero variation. These predictors would add little benefit to include in models.
...
The training was then divided to create a validation set which will be used for
...
The data method is also more efficient than using the formula method. 
