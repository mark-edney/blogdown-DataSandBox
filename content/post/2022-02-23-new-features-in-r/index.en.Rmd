...
From this example, it is apparent that the behavior of the native pipe is the same as the magrittr pipe.
...
There has been a simplification in the creation or lambda functions. The notation is simplified while the results are the same.
...
* The default has been changed for 'stringsAsFactors = FALSE'. Previously, when using the data.frame() or the read.table() the default option would turn strings into factors. This was a annoying feature that would always create headaches.
...
* c() can now combine factors to create a new factor. I am not familiar with the original behavior but this seems intuitive.
