...
The creation of a corpus is just the start. Models need to be created in order to do the actually prediction work. The main idea is to match the previous set of words with the words that make up the ngrams. The final word in the ngram is than the prediction. The prediction takes the form of a distribution as there are likely multiple matches in the corpus. Different models change the distribution and thus the propability of a word becoming the predicted word. This distributin is created by taking a count of all the matches in the corpus. Each count is than normalized by the total count of every possible match to produce a probabilty that adds to one. 

### Maximum Likelihood  Estimate

`$SE = \sqrt{\frac{s_{1}^{2}}{n_{1}}+\frac{s_{2}^{2}}{n_{2}}}$`

The Maximum Likelihood Estimate (MLE) is the simplest algorithm for examining the distribution. It simply returns the value with the highest count/occurrences in the corpus. There are some issues with this method as for many words in the Vocabulary there are no occurrences and therefore no probability of the word being estimated. This is undesirable as we would like some probability assigned with every possible word.

### Add One Smoothing

Add one smoothing is 
