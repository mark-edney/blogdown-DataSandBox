...
This is the most basic model that I would like to evaluate. I've used the `GridSearch` function so all combinations of parameters are tested. The only parameter I wanted examine was the size of the hidden layers. Each hidden layer provided is a tuple were each number represents the number of nodes in a singled layer. Multiple numbers represent additional layers.  
...
grid.best_score_
```
The performance of the scaled model is much more impressive. After the `GridSearch` function finds the parameters for the best model, it retrains the model on the entire dataset. This is because the function utilize cross validation, so some of the data was withheld for comparing the different models on test data.
