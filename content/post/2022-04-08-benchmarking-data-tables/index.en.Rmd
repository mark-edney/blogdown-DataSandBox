...
The '.()' function can be used as a place holder for 'list()'. The list function is useful for subsetting.
...
Another function of interest is the '.N' function. This function will return the count of rows. The test looks are the number of people with over 5000 in credit and younger then 35. 
...
Data Tables also contain a very simple syntax for creating a new column with ':='. I compare this to the `tidyverse` mutate function. Using the base R to create a column is still the fastest  method, taking about have the time of the Data Table method. 
...
```
I don't think this is the most useful feature as you can already create some very complicated transformation with the a single call. Chaining also makes it more difficult to understand. 
...
```
It is clear that there is significant performance improvements when using Data Tables versus Data Frames
(an average decrease of time by `r total`%). There is also insignificant differences between Data Frames and Tibbles. Also the syntax for Data Tables is fairly simple and straight forward and yet extremely powerful.
