...
My shiny dashboard will at heart be a shiny application. I will use the single App file structure rather then the separate UI/Server file structure. The R Studio site has a very good outline on the basic structure for a [Shiny Dashboard](https://rstudio.github.io/shinydashboard/get_started.html). 
...
The data is the same from the previous dashboard which was the level of donations for Canadians to charities. The data can be found [here](https://open.canada.ca/data/en/dataset/74c77af4-73c4-4e0d-ac5d-f74a247cdf12). 
...
# The fluidRow function is used to align the elements on the page. Through trail and error, I have found this to be the element for changing the size of plots.
                                fluidRow(
...
The server is the second major function in the Shiny App. Rather than using the output series of functions, it requires the paired version of the render function. Any controls from the UI would need to be pass as part of the `input` variable. 
...
The finalized dashboard looks pretty good. The sidebar add a good level of control to the user and makes the data better organized. I also enjoy the default styling but I'm sure it can be changed.

When compared to the dashboard created with `flexdashboard`, it much nicer looking. The flexdashboard however, has the advantage of being much easier to create with r markdown. It is also much more flexible as it produces an html file which is easy to manage. The shiny dashboard needs to be hosted which makes it slightly clunky. 

Speaking of the clunky nature of Shiny Apps, you also need to be concise of what needs to be reactive and what doesn't. It is very easy to slow down a Shiny Dashboard with a bunch of unnecessary calculations. The writing of the application itself is also quite clunky. It doesn't compare to the ease of writing in R markdown. 

So which dashboard do I prefer? Well I think for a quick solution, flex dashboard is a great but the amount of control and styling of a shiny dashboard is just better. I do think that there is a place for both, but if I had to choose I would go for a Shiny Dashboard. 
