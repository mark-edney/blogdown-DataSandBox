...
It may appear from first impressions that a list comprehension is a simple one line for loop, but it is much more powerful then that. 
...
But what if you need create an even more complicated list, one that requires an else statement along with the if statement. Then the structure of the list comprehension changes a little, the iterable statement is moved to the end. 
...
Of course expressions can be more complicated then returning single values. One common issue I find is when I have a list of a value type and I need them to be of a different type. This conversion is easy with list comprehensions. 
...
```
Likewise, you can create a set rather then a list. Sets can be useful if you don't need the data to be in an order and you don't want any duplicate values. 

### Other Applicatons

There is great potential in list comprehensions. Often I find the I need to create a list of zeroes or of boolen logic of the same size of a current list. This is easy to create, just dont refer to to the iterable within the expression. 
