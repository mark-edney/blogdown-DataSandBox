...
```
An ngram is continuous sequence of tokens, where the order is determined by how many tokens are in the sequence. For our purpose, a token is created for each word in a sentence. Other tokens can be created, such as sentence in a paragraph or letters in a word. It really depends on your application needs. 

A line of text can be broken down into ngrams many different ways. For example, the following text:
"The quick brown fox" 
...
### Add One Smoothing
The simplest way to deal with the issue of zero probability values is to add one to all unseen counts. This is also referred to as Leplace Smoothing. Due
$$p_x = \begin{cases}
\frac{C_x}{C} & C_x > 0 \\
\frac{1}{C} & C_x = 0
\end{cases}
...
The plot for the add one model is pretty easy to create from the previous sample. It is clear that there is some values now in each bin, so there is some probability to every word in the vocabulary. The heights of the bins are also increased as there previously were words in each bin that had 0 occurances now occurring once. 
...
### Good Turing
In order to understand the Good Turing Smoothing we need to introduce some new notation, $N_C$, to represent the frequency of frequencies. The frequency of frequencies represents how often a number of occurrences will happen in or distribution. For example, $N_0$ represents the word count in our vocabulary where there is on occurrences of that word in the distribution. $N_1$ than represents the count of the words that have one occurrence. The frequency of frequencies is a one layer of abstraction from our counts. It helpful to consider our previous plots were we created bins based on the first letter of the predicted word, but instead we are creating bins one how often our predicted words occur.
...
The first intuition of the Good Turing is that the probability of something new, a word with a count of zero, should be assigned the probability for an event that occurred once. For this example, we have the very unlikely event that the are no counts of words that appear once so we use the next available count(X). This will give the probability of all words with zero count, we will later divide it by the number of words with the count 0.  
...
The next code chunk can look quite complicated. In this chunk, the corrected count, $C^*$, are calculated. The variable j is used to control whether the the regression model is used to substitute the value for $N_{C+1}$.  
...
The probabilities at this time need two additional modifications, they need to be normalized as the regression model skews the overall probability and the probabilities need to be divide by the frequency counts to get a word specific probability.
...
We can now plot the completed ngram prediction for the Good Turing Smoothing. The plot looks similar to previous plots, but we plot the probabilities rather then the count values 'n'.
...
Good Turing Smoothing is a effective model but man can it be complicated. One observation that you can make when looking at the values for $C$ and $C^*$ is that there is nearly constant discounting. The distribution in our example is skewed, but we can see that the most common value is between 0 and 1.  
...
$$
where 'd' is the discounting amount, $\lambda$ is the Interpolation rate and $p_{unigram}$ is the unigram probability based one the MLE.
...
The following code is used to determine the continuation probabilities. Since the highest order ngram is six, the continuation probability needs to be calculate for six different ngrams.
...
```
The difficulty is with finding the continuation probability. After they are found, its pretty easy to add them to the model. 
...
## Shiny App
With all of the models created, we can bundle it together in a single Shiny Application. This Shiny Application retrieves the users input an attempts to predict the next word. A table is generated to summarise the most highly predicted word. Since there are five different models, there are five different rows. A plot is generated for each model where the predicted words are in bins with other words with the same first letter.  
