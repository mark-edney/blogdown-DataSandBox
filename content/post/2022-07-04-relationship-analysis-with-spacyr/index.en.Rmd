...
As a reminder, this project was inspired by the work of [Thu Vu](https://www.youtube.com/watch?v=RuNolAh_4bU) were she created a network mapping of the characters in the Witcher series. I thought it would be interesting to do some recreation of this project but in `R` and with the **Stormlight Archive** book series. 
...
With the books read into file, the corpus created and the corpus split into sections, we now have `r length(corpus)` documents. We can proceed to entity modeling with the `spaCy` functions.
...
This is another very time consuming tasks the requires two for loops. The first loop goes through all `r nrow(dfclean)` rows and its sentence id. A second for loop that excludes all rows all ready used in the first loop is used to compare a second sentence id. If the difference between the senetence ids is less than the windows, the tokens for these rows is added to a empty data table. If the difference is greater than the window size, we break the second for loop as all the sentence ids are incremental. It is not a very clear or smooth method but it works. 
